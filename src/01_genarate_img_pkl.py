...
def genarate_pickle_file(config_path,params_path):
    ...
    for actor in actors:
        for file in os.listdir(os.path.join(data_path,actor)):
            file_names.append(os.path.join(data_path,actor,file))

    logging.info(f'Total actress are: {len(actors)}')
    logging.info(f'Total actress images: {len(file_names)}')
    pickle.dump(file_names, open(pickle_file, 'wb'))
...
if __name__ == '__main__':
    ...
    try:
        ...
        genarate_pickle_file(config_path=parsed_args.config, params_path=parsed_args.params)
